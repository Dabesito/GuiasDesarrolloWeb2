...
export const ExpenseForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        // Ejercicio Complementario 1: Validar presupuesto
        const totalExpenses = state.expenses.reduce((total, exp) => total + exp.amount, 0);
        const available = state.budget - totalExpenses;
        
        // Si estamos editando, calculamos la diferencia. Si es nuevo, es el monto completo.
        const difference = state.editingId 
            ? expense.amount - state.expenses.find(current => current.id === state.editingId).amount 
            : expense.amount;
